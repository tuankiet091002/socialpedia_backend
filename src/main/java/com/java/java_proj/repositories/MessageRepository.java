...
public interface MessageRepository extends JpaRepository<Message, Integer> {
    ...
    Page<DResponseMessage> findByChannel(String content, Channel channel, Pageable pageable);

    @Query("SELECT m FROM Message m " +
            "WHERE m IN (SELECT i.messages FROM Inbox i WHERE i = :inbox) " +
            "AND m.content LIKE CONCAT('%', :content, '%')")
    Page<DResponseMessage> findByInbox(String content, Inbox inbox, Pageable pageable);
